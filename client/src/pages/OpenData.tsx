...
const API_BASE = import.meta.env.VITE_API_URL ?? ""; // leave empty if reverse proxy routes /api to open-data

type PdfImageCardProps = {
    title: string;
    imgSrc: string;
    imgAlt: string;
    pdfPath: string; // e.g. "/api/opendata/dorms.pdf"
    page?: number;
    pageSize?: number;
    disabled?: boolean;
...
const OpenDataPage: React.FC = () => {
    ...
    // which PDF is selected to preview inline
    ...
    const onView = useCallback((args: { title: string; pdfPath: string }) => {
        setSelected(args);
        // The iframe will compute the URL from current page/pageSize
    }, []);
            ...
            <div className="mx-auto max-w-6xl px-4 py-10">
                ...
                <div className="mt-10">
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
                        {selected ? (
                            <iframe
                                key={selectedUrl} // force refresh when params change
                                src={selectedUrl}
